...
def args_dict(args):
    ...
    args.setting = {'segment':args.segment}
    codebook_dim = args.hidden * 2**args.depth
    args.VAE  = {'hidden':args.hidden, 'codebook_dim':codebook_dim, 'codebook_size':args.codebook_size,
...
def torch_sisdr(reference, estimation):
    ...
    return torch.mean(10 * torch.log10(ratio),dim=1 ,keepdim=True)
