...
class Log():
    ...
    def add_plot_image(self, cate:str, global_step:int, **kwargs):
        for k, v in kwargs.items():
            ...
            plt.imshow(v)
            plt.colorbar()
            plt.savefig(os.path.join(self.cache_dir, "temp.png"), dpi=300)
            plt.close()
            image = PIL.Image.open(os.path.join(self.cache_dir, "temp.png"))
            self.writer.add_image(tag=f"{cate}/{k}_plot",
                                  img_tensor=np.array(image),
                                  global_step=global_step,
                                  dataformats="HWC")
  
    def add_image(self, cate:str, global_step:int, dataformats:str="HW", **kwargs):
    ...
    def add_audio(self, cate:str, global_step:int, add_spec:bool = True, **kwargs):
        ...
        for k, v in kwargs.items():
            self.writer.add_audio(tag=f"{cate}/{k}_audio",
                                  ...
                                  sample_rate=16000)
            if add_spec:
                spec_imags[k] = self.to_spec(v[0] if v.dim() > 1 else v)
